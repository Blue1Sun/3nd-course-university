...
where ((c.Doctor = a.Doctor) AND (c.Reg_date = a.Date_Time) AND (a.Pet = p.Id) AND (c.Diagnose = d.id) AND (d.name = 'ßùóð'));
...
where ((ds.Symptom = s.Id) AND (ds.Diagnose = d.Id) AND (s.Name like 'òîøíîòà'));
...
where ((dhead.role = 'ðóêîâîäèòåëü') AND (dhead.login = 'gudin') AND (dhead.Depart = d.Number) AND (dc.Depart = d.Number));
